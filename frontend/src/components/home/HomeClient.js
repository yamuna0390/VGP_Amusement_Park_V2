  ...
  const fallbackReviews = [
    {
      id: "fb1",
      rating: 5,
      review_text: "One of those rare times when you won’t say ‘silence’ when the kids are shouting!",
    ...
    },
    {
      id: "fb2",
      rating: 5,
      review_text: "Took my school group of 60 kids — the Chutti Sepoys kept everyone entertained all day.",
    ...
    },
    {
      id: "fb3",
      rating: 4,
      review_text: "Rodeo Ride and Flying Machine were the highlights. The beach at sunset is magical.",
          ...
          <div className="polaroid-grid">
            <Link href="/rides/power-surge" style={{textDecoration:'none'}}>
              <div className="polaroid r-l">
                <img src="/assets/power surge.png" alt="Top Gun" loading="lazy"/>
                <span className="ribbon">Power Surge </span>
                <div className="note">
            ...
            <div className="grid g3">
              {activeReviews.map((review, index) => (
                <div 
                  className="review" 
                  key={review.id}
                  style={index % 2 !== 0 ? { transform: 'rotate(1.2deg)' } : {}}
                >
                  <div className="stars">
                    {"★".repeat(review.rating)}{"☆".repeat(5 - review.rating)}
                  </div>
                  <p>&ldquo;{review.review_text}&rdquo;</p>
                  <div className="who">— {review.customer_name}{review.location ? `, ${review.location}` : ""}</div>
                </div>
              ))}
            </div>
          ) : (
            <div className="reviews-carousel-container" style={{ position: 'relative', padding: '0 44px', marginTop: '10px' }}>
              <SharedSlider
                items={activeReviews}
                autoplayDelay={4000}
                speed={650}
                loop={true}
                navigationProps={{ nextEl: '.reviews-next', prevEl: '.reviews-prev' }}
                breakpoints={{
                ...
                slidesPerView={1}
                spaceBetween={20}
                swiperClassName="reviews-swiper-wrapper"
                renderItem={(review, index) => (
                  <div 
                    className="review" 
                    style={{
                      ...(index % 2 !== 0 ? { transform: 'rotate(1.2deg)' } : {}),
                      margin: '20px 0' // Ensures shadows aren't clipped by the native overflow:hidden edge
                    }}
                  >
                    <div className="stars">
                      {"★".repeat(review.rating)}{"☆".repeat(5 - review.rating)}
              ...
              <div 
                ...
                style={{
                  ...
                  cursor: 'pointer', zIndex: 10, boxShadow: '0 4px 12px rgba(0,0,0,.35)'
                }}
              >
